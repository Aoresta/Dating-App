        ...
        <div className="glass-card mt-7 p-5">
          <div className="mb-5 grid grid-cols-2 rounded-full bg-white/5 p-1">
            {['Sign In', 'Sign Up'].map((x, i) => (
              <button key={x} onClick={() => setSignup(Boolean(i))}
          ...
          <form onSubmit={submit} className="space-y-3">
            {signup && <input className="field" placeholder="Your name" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} />}
            ...
            <input className="field" type="password" required minLength="6" placeholder="Password" value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} />
            <button className="primary-btn w-full" disabled={loading}>
              {loading ? 'Please wait...' : signup ? 'Create Account' : 'Sign In'}
            </button>
          </form>
          {msg && <p className="mt-3 text-center text-xs text-pink-light">{msg}</p>}
